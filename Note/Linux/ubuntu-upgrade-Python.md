...
默认版本和升级后执行 `pip list` 均报错，没有按照 `pip` 工具。因此，升级后执行命令 `sudo apt-get install python3-pip` 安装 `pip` 如果使用 `sudo apt-get install python-pip` 则出错。

```bash
ubuntu@VM-0-4-ubuntu:~$ pip list
chardet (2.3.0)
